...
RMenu.Add('hs0_bank', 'bank', RageUI.CreateMenu("~y~Compte", "~y~Voici votre compte en banque."))
RMenu:Get('hs0_bank', 'bank'):SetRectangleBanner(0, 0, 0, 255)
...
Citizen.CreateThread(function()
    while true do
        ...
        RageUI.IsVisible(RMenu:Get('hs0_bank', 'bank'), true, false, true, function()        

            RageUI.Separator("~g~ Banque Crée pour FiveLifeRP")
            RageUI.Separator("~g~ " ..argentsolde.."~g~ $")

            RageUI.ButtonWithStyle("Refresh le Solde", nil, {RightLabel = "→→→"},true, function(Hovered, Active, Selected)
            if (Selected) then   
                verifsolde()
                Wait(400)
            end
            ...
            RageUI.ButtonWithStyle("Déposer", nil, {RightLabel = "→→→", Color = { BackgroundColor = { 93, 182, 229, 255 } }},true, function(Hovered, Active, Selected)
            if (Selected) then   
                saisiedepot()
            end
            end)

            RageUI.ButtonWithStyle("Retirer", nil, {RightLabel = "→→→", Color = {  BackgroundColor = { 224, 50, 50, 255 } }},true, function(Hovered, Active, Selected)
            if (Selected) then   
                saisieretrait()
            end
            end) 

            RageUI.Separator("~g~ © FlashJoJo | 2021")
            ...
            for k in pairs(position) do
                ...
                if dist <= 1.0 then
                    ...
                    if IsControlJustPressed(1,51) then
                    	verifsolde()
                        RageUI.Visible(RMenu:Get('hs0_bank', 'bank'), not RageUI.Visible(RMenu:Get('hs0_bank', 'bank')))
                    end   
                end
            end
        end
